...
----
# CRDs (global scope)
kubectl apply -k "github.com/projectsyn/lieutenant-operator/deploy/crds?ref=v0.3.0"

# Operator deployment
kubectl -n lieutenant apply -k "github.com/projectsyn/lieutenant-operator/deploy?ref=v0.3.0"
...
----
# API deployment
kubectl -n lieutenant apply -k "github.com/projectsyn/lieutenant-api/deploy?ref=v0.3.0"
...
. Patch the Tenant object directly in Kubernetes to add a Cluster template.
+
[source,shell]
----
...
Get the k3d Kubernetes API URL with `k3d kubeconfig get projectsyn` and replace `REPLACE_API_URL` in the command below:
[source,shell]
----
