...
The installation of Steward happens on the K3s cluster "k3d-steward" via an install URL provided by Lieutenant, using a one-time bootstrap token. The token is valid only once and for 30 minutes after cluster registration.
...
After these steps, the local K3s cluster is now Syn enabled, and a Git repository called "Project Syn Cluster" is available in GitLab. But it's worth pointing out that Argo CD hasn't yet synchronized any changes.
