...
[NOTE]
====
Repositories of Project Syn provide an https://editorconfig.org/c[EditorConfig].
When using a supported editor, space types and number of spaces will automatically be done correctly.
====
...
* Variables should be named in camelCase style, and should have
self-evident names.
+
[source,jsonnet]
----
...
* Jsonnet files which requires parameters to be materialized or which
are libraries should end with the `.libjsonnet` suffix.
* Files in `lib` always are libraries and must be named accordingly.
  Those files are considered part of a public API.
  Treat classes and functions accordinglly and lookout for braking changes.
