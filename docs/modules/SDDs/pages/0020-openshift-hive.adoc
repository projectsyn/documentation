...
In a first PoC phase, all information that's not yet present in a cluster's facts will be provided in annotations on the cluster object:
...
* `hive.syn.tools/worker-count` Count of worker nodes to provision
* `hive.syn.tools/worker-type` Instance type to use for worker nodes

`*` are required for provisioning
