...
Tools must have descriptive display names of the form `Syn ${type}[ ${purpose}]`.
By default the binary name for a tool would be `syn-$type[-$purpose]`, but deviations are allowed, see below.
...
=== Allowed deviations between tool and binary name

A tool's binary is allowed to deviate as long as it is discoverable with `syn<Tab><Tab>`, and the suffix is well known as a particular type of tool.

Examples where deviation is OK:
