...
Dynamic facts are information about a Syn managed Cluster which can be determined form the state of the cluster and may change regularly.
...
The facts can then be accessed either through the REST API or directly form the Cluster resource.

=== Fact Collection

Periodically Steward collects all dynamic facts in its cluster. 
...
It will store it in the Cluster resources status.
...
Dynamic facts are a prime example of a cluster state and as such should be store in the status subresource.
...
It's generally cleaner when the controller itself gets fetches the status of its managed resources instead of an external API pushing state into its resources.
...
An alternative to storing the facts in the Cluster resource is to put it into a timeseries DB.
