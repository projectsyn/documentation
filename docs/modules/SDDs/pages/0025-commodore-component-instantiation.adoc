...
There are already a some components for which installing multiple instances is a common use case, such as the https://github.com/projectsyn/component-nfs-client-provisioner/[nfs-client-provisioner component].
...
Component authors must explicitly mark their components as supporting instantiation.
Similar to Helm charts, the components themselves must make sure to not cause any naming collisions of objects
This is required both for namespaces and non-namespaced resources.
