...
:sdd_reviewers: Aldebaran, Schedar
:sdd_date:      2023-05-02
:sdd_status:    draft
...
The bootstrap process for the additional root applications is initiated by defining the team in the configuration hierarchy in parameter `syn_teams`.
...
For each team, the list of component instances owned by that team is specified in `syn_teams.<team-name>.instances`.
...
This change can be implemented globally in the component template.
By implementing the dynamic apps path in the catalog in the component template, we don't need to adjust Commodore's logic for copying the output of `kapitan compile` into the cluster catalog.
