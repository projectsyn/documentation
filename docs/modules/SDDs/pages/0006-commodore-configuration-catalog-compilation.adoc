...
The target pulls in a generic inventory classes for global VSHN configuration (class `global.common` ), cloud-specific configuration (for example class `global.cloud.aws` ) and cluster distribution specific configuration (for example `global.distribution.rancher` ).
...
Secret references can use reclass references to define dynamic defaults, for example `?{vaultkv:${customer:name}/${cluster:name}/thesecret/thekey}`.
