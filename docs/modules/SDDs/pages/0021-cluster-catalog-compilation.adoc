...
* Define where (in terms of which cluster, namespace, etc.) the jobs are run
...
In a first PoC phase this can be an annotation like `cluster.syn.tools/needs-compilation=true`.
...
To distribute the load this compilation generates and to follow a decentralized approach, these jobs are run on the same cluster as they compile the catalog for.
The Steward component on a cluster starts a compilation job if the `cluster.syn.tools/needs-compilation` annotation is set to `true` on its own cluster object.
It will then launch the respective job to compile the cluster catalog.
...
For example could Quartermaster make sure that a certain amount of compile jobs per amount of time isn't exceeded for a single cluster or tenant.
...
Such a solution would probably require a centralized
