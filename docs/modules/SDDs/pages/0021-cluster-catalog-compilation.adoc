...
To distribute the load the compilation generates and to follow a decentralized approach, these jobs are run on the same cluster as they compile the catalog for.
By default the compilation engine creates the compile jobs on the respective target clusters (for example via Steward).
