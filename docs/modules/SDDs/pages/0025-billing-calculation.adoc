...
Customer ID:: Stored in label `customer_id`
Product ID:: Per billed unit / invoice line, stored in label `product_id`
...
.Example sample
----
billed_unit_amount{customer_id="2354334",lieutenant_cluster_id="cluster-42",product_id="35345"} 8
billed_unit_price{customer_id="2354334",lieutenant_cluster_id="cluster-42",product_id="35345"} 0.10
----
...
`kube_node_status_capacity_cpu_cores` with the identifier label `lieutenant_cluster_id`
...
`lieutenant_tenant` matching via label `lieutenant_cluster_id`, contains label `customer_id`

Price model metric::

`billed_unit_price` matching via labels from the result of the above matching, contains label `product_id`.
...
`managed_database_cpus` with the identifier label `lieutenant_cluster_id`, `customer_id` and all needed labels for price matching.
...
`billed_unit_price` matching via labels from the source metric, contains label `product_id`.

Invoice line sample::

----
billed_unit_amount{customer_id="2354334",lieutenant_cluster_id="cluster-42",product_id="35345"} 10
billed_unit_price{customer_id="2354334",lieutenant_cluster_id="cluster-42",product_id="35345"} 0.20
----
====
